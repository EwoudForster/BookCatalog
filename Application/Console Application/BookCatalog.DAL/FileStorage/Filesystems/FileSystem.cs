...
namespace BookCatalog.DAL.Storage.FileStorage.Filesystems
{
    ...
    public class FileSystem<T> : IFileSystem<T> where T : class, IEntity
    {
        ...
        public FileSystem(string fileName, ISerialize<T> serializer)
        {
            try
            {
                _serializer = serializer ?? throw new ArgumentNullException(LoggingStrings.ErrorCreatingRepository(serializer.GetType().Name));

                // the file path is created using the file name
                _filePath = FilePath(fileName) ?? throw new ArgumentNullException(LoggingStrings.ErrorCreatingRepository(serializer.GetType().Name)); ;
            }
            catch (Exception)
            {
                throw;
