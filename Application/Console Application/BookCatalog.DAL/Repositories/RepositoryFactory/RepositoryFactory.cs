...
namespace BookCatalog.DataLayer.Repositories.RepositoryFactory
{
    public class RepositoryFactory<T> where T : class, IEntity, new ()
    {
        ...
        private void GetRepository()
        {
            ...
            // Load the assembly
            var loadContext = new LoadContext(repositoryLocation);
            Console.WriteLine($"loadcontext: {loadContext.ToString()}");

            var assemblyName = new AssemblyName(Path.GetFileNameWithoutExtension(repositoryAssemblyName));
        ...
        private void GetFileSystem()
        {
            ...
            // Load the assembly
            var loadContext = new LoadContext(fileSystemLocation);
            Console.WriteLine($"loadcontext: {loadContext.ToString()}");

            var assemblyName = new AssemblyName(Path.GetFileNameWithoutExtension(fileSystemAssemblyName));
            Console.WriteLine($"assemblyName: {assemblyName.ToString()}\n");
            ...
            {
                Type genericFileSystemType = fileSystemType.MakeGenericType(typeof(Book));
        ...
        private void GetFormatter()
        {
            ...
            // Load the assembly
            var loadContext = new LoadContext(formatterLocation);
            Console.WriteLine($"loadcontext: {loadContext.ToString()}");

            var  assemblyName = new AssemblyName(Path.GetFileNameWithoutExtension(formatterAssemblyName));
            Console.WriteLine($"assemblyName: {assemblyName.ToString()}\n");
            ...
            {
                Type genericFormatterType = formatterType.MakeGenericType(typeof(Book));
