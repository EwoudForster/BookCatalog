...
namespace BookCatalog.DAL.Services
    {
    public class BookService{
        ...
        public BookService(IRepository<Book> repository, IGeneralLogger logger)
        {
            try
            {
                // Making a logger to log the actions
                ...
                _repository = repository ?? throw new ArgumentNullException(nameof(repository), LoggingStrings.ErrorNullArgument(nameof(repository)));
            }
            catch (Exception ex)
            {
                _logger.Error(ex, LoggingStrings.ErrorCreatingRepository(nameof(BookService)));
                throw;
            }
        }
                ...
                while (continueOperation)
                {
                    ...
                    switch (choice.ToLower())
                    {
                        ...
                        case var st when choice.Contains("genre"):
                            ...
                            break;
                        case "8":
                        case var st when choice.Contains("exit"):
                            // redirect to the group by genre method
