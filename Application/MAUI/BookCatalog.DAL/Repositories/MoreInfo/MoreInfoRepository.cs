...
{
    public class MoreInfoRepository : GenericRepositoryAsync<MoreInfo>, IMoreInfoRepository
    {

        public MoreInfoRepository(BookCatalogDbContext context, ILogger<MoreInfoRepository> logger) : base(context, logger)
        {
        }

        // Get all entities with count of books per MoreInfo
        public async Task<IEnumerable<MoreInfoCalculated>> GetAllWithBookStats()
        {
            try
            {
                ...
                var result = await _dbSet
                    .Select(MoreInfo => new MoreInfoCalculated
                    {
                        Id = MoreInfo.Id,
                        Name = MoreInfo.Name,
                        Books = MoreInfo.Books,
                        Average = MoreInfo.Books.Any() ? MoreInfo.Books.Average(b => b.Price) : 0m,
                        Max = MoreInfo.Books.Any() ? MoreInfo.Books.Max(b => b.Price) : 0m,
                        Min = MoreInfo.Books.Any() ? MoreInfo.Books.Min(b => b.Price) : 0m,
                    })
                    .ToListAsync();
        ...
        public async Task<Dictionary<StatisticsOptions, object>> GetStatisticsForBooksPerMoreInfo(Guid MoreInfoId, BookByStatsOptions option = BookByStatsOptions.Price)
        {
            try
            {
                ...
                var StatisticsType = "€";
                // variable to store the query
                Func<IQueryable<MoreInfo>, IQueryable<decimal>> query = x => x.Where(p => p.Id == MoreInfoId).SelectMany(b => b.Books).Select(b => b.Price);

                // Checking the option and getting the statistics by the option
                switch (option)
                {
                    case BookByStatsOptions.Year:
                        query = x => x.Where(p => p.Id == MoreInfoId).SelectMany(b => b.Books).Select(b => (decimal)b.PublicationYear);
                        StatisticsType = "Publication year";
                        break;

                    case BookByStatsOptions.Page:
                        query = x => x.Where(p => p.Id == MoreInfoId).SelectMany(b => b.Books).Select(b => (decimal)b.PageCount);
                ...
                statistics.Add(StatisticsOptions.TotalBooks, await _dbSet.Where(g => g.Id == MoreInfoId).Select(b => b.Books).CountAsync());
