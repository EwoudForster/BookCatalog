...
public class GenericRepositoryAsync<T> : IRepository<T> where T : class, IEntity, new()
{
    ...
    protected readonly BookCatalogDbContext _dbContext;
    ...
    public GenericRepositoryAsync(BookCatalogDbContext DbContext, ILogger<GenericRepositoryAsync<T>> logger)
    {
        try
        {
            ...
            // DbContext declaration and checking if not null
            _dbSet = DbContext.Set<T>() ?? throw new ArgumentNullException(nameof(_dbSet), LoggingStrings.ErrorNullArgument("DbSet"));
    ...
    // Get all entities
    public async Task<IEnumerable<T>> GetAll(bool sortCreatedAt = false)
    {
        try
        {
