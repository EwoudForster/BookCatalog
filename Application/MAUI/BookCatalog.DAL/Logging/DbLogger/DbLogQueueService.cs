...
public class DbLogQueueService : BackgroundService
{
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly Channel<LoggingEntry> _logChannel;

    public DbLogQueueService(IServiceScopeFactory scopeFactory, Channel<LoggingEntry> logChannel)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        await foreach (var log in _logChannel.Reader.ReadAllAsync(stoppingToken))
        {
            try
            {
                ...
                var repo = scope.ServiceProvider.GetRequiredService<IRepository<LoggingEntry>>();
                await repo.Add(log);
            }
            catch (Exception ex)
            {
